...
feature 'Promotion with option value rule' do
  ...
  given!(:greetingcard) { variant.greetingcard }
  ...
  scenario "adding an option value rule", js: true do
    ...
    within("#rules .promotion-block") do
      ...
      expect(page.body).to have_content("Greetingcard")
    ...
    within('.promo-rule-option-value') do
      ...
      targetted_select2_search greetingcard.name, from: '.js-promo-rule-option-value-greetingcard-select'
      targetted_select2_search(
        option_value.name,
        from: '.js-promo-rule-option-value-option-values-select'
        )
    ...
    expect(first_rule.preferred_eligible_values).to eq Hash[greetingcard.id => [option_value.id]]
  end
  ...
  context "with an existing option value rule" do
    ...
    background do
      ...
      rule.preferred_eligible_values = Hash[
        variant1.greetingcard_id => variant1.option_values.pluck(:id),
        variant2.greetingcard_id => variant2.option_values.pluck(:id)
      ]
      rule.save!
    ...
    scenario "deleting a greetingcard", js: true do
      within(".promo-rule-option-value:last-child") do
        find(".delete").click
      end
      ...
      first_rule = promotion.rules(true).first
      expect(first_rule.preferred_eligible_values).to eq(
        Hash[variant1.greetingcard_id => variant1.option_values.pluck(:id)]
      )
    end
  end
end
