...
module Spree
  module Core
    describe Importer::Order do
      ...
      let(:greetingcard) do
        greetingcard = Spree::Greetingcard.create(name: 'Test',
                                        sku: 'TEST-1',
                                        price: 33.22, available_on: Time.current - 1.day)
        greetingcard.shipping_category = create(:shipping_category)
        greetingcard.save
        greetingcard
      end
      ...
      context "shipments" do
        ...
        it 'builds them properly' do
          ...
          expect(shipment.inventory_units.first.variant_id).to eq greetingcard.master.id
