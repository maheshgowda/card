  ...
  describe Api::V1::ImagesController, :type => :controller do
    ...
    context "as an admin" do
      ...
      it "can learn how to create a new image" do
        ...
        api_get :new, greetingcard_id: greetingcard.id
        ...
        expect do
          api_post :create,
                   :image => { :attachment => upload_image('thinking-cat.jpg'),
                               ...
                               :viewable_id => product.master.to_param, greetingcard.master.to_param  },
                   :product_id => product.id
                   :greetingcard_id => greetingcard.id
      ...
      context "working with an existing image" do
        ...
        let!(:greetingcard_image) { greetingcard.master.images.create!(:attachment => image('thinking-cat.jpg')) }
        ...
        it "can get a single greetingcard image" do
          api_get :show, :id => greetingcard_image.id, :greetingcard_id => greetingcard.id
          expect(response.status).to eq(200)
          expect(json_response).to have_attributes(attributes)
        end
        ...
        it "can get a single variant image" do
          ...
          api_get :show, :id => greetingcard_image.id, :variant_id => greetingcard.master.id
          expect(response.status).to eq(200)
          expect(json_response).to have_attributes(attributes)
        end
        ...
        it "can get a list of greetingcard images" do
          api_get :index, :greetingcard_id => greetingcard.id
          expect(response.status).to eq(200)
          expect(json_response).to have_key("images")
          expect(json_response["images"].first).to have_attributes(attributes)
        end
        ...
        it "can get a list of variant images" do
          ...
          api_get :index, :variant_id => greetingcard.master.id
          expect(response.status).to eq(200)
          expect(json_response).to have_key("images")
          expect(json_response["images"].first).to have_attributes(attributes)
        end
        ...
        it "can update image data" do
          ...
          expect(greetingcard_image.position).to eq(1)
          api_post :update, :image => { :position => 2 }, :id => product_image.id, :product_id => product.id
          api_post :update, :image => { :position => 2 }, :id => greetingcard_image.id, :greetingcard_id => greetingcard.id
          expect(response.status).to eq(200)
          expect(json_response).to have_attributes(attributes)
          expect(product_image.reload.position).to eq(2)
          expect(greetingcard_image.reload.position).to eq(2)
        end
        ...
        it "can't update a image without attachment" do
          api_post :update,
                   ...
                   id: greetingcard_image.id, greetingcard_id: greetingcard.id
          expect(response.status).to eq(422)
        end

        it "can delete an image" do
          api_delete :destroy, :id => product_image.id, :product_id => product.id
          api_delete :destroy, :id => greetingcard_image.id, :greetingcard_id => greetingcard.id
          expect(response.status).to eq(204)
          expect { product_image.reload }.to raise_error(ActiveRecord::RecordNotFound)
          expect { greetingcard_image.reload }.to raise_error(ActiveRecord::RecordNotFound)
        end
      end
    end
    ...
    context "as a non-admin" do
      it "cannot create an image" do
        api_post :create, :product_id => product.id, :greetingcard_id => greetingcard.id
        assert_unauthorized!
      end

      it "cannot update an image" do
        api_put :update, :id => 1, :product_id => product.id, :greetingcard_id => greetingcard.id
        assert_not_found!
      end

      it "cannot delete an image" do
        api_delete :destroy, :id => 1, :product_id => product.id, :greetingcard_id => greetingcard.id
        assert_not_found!
      end
    end
  end
