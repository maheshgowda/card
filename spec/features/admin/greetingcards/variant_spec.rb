...
describe "Variants", type: :feature do
  ...
  let(:greetingcard) { create(:greetingcard_with_option_types, price: "1.99", cost_price: "1.00", weight: "2.5", height: "3.0", width: "1.0", depth: "1.5") }

  context "creating a new variant" do
    it "should allow an admin to create a new variant", js: true do
      greetingcard.options.each do |option|
  ...
  context "listing variants" do
    context "currency displaying" do
      context "using Russian Rubles" do
        ...
        let!(:variant) do
          create(:variant, greetingcard: greetingcard, price: 19.99)
        end

        # Regression test for #2737
        context "uses руб as the currency symbol" do
          it "on the greetingcards listing page" do
            visit spree.admin_greetingcard_variants_path(greetingcard)
