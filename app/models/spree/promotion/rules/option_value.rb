    ...
    module Rules
      class OptionValue < PromotionRule
        ...
        def actionable?(line_item)
          ...
          pid = line_item.greetingcard.id
          ovids = line_item.variant.option_values.pluck(:id)

          greetingcard_ids.include?(pid) && (value_ids(pid) - ovids).empty?
        end
        ...
        def value_ids(greetingcard_id)
          preferred_eligible_values[greetingcard_id]
        end
      end
    end
  end
