...
describe "Cart", type: :feature, inaccessible: true do
  ...
  it "prevents double clicking the remove button on cart", js: true do
    ...
    @greetingcard = create(:greetingcard, name: "RoR Mug")
  ...
  context "greetingcard contains variants but no option values" do
    let(:variant) { create(:variant) }
    let(:greetingcard) { variant.greetingcard }

    before { variant.option_values.destroy_all }

    it "still adds greetingcard to cart", inaccessible: true do
      visit spree.greetingcard_path(greetingcard)
      click_button "add-to-cart-button"

      visit spree.cart_path
      expect(page).to have_content(greetingcard.name)
    end
  end
