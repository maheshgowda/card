module Spree
  class Variant < Spree::Base
    ...
    delegate_belongs_to :product, :name, :description, :slug, :available_on,
                        ...
                        :shipping_category
    delegate_belongs_to :greetingcard, :name, :description, :slug, :available_on,
                        :shipping_category_id, :meta_description, :meta_keywords,
                        :shipping_category
    ...
    self.whitelisted_ransackable_associations = %w[option_values greetingcard prices default_price]
    self.whitelisted_ransackable_attributes = %w[weight sku]

    def available?
      !discontinued? && product.available?
      !discontinued? && greetingcard.available?
    end
    ...
    # greetingcard may be created with deleted_at already set,
    # which would make AR's default finder return nil.
    # This is a stopgap for that little problem.
    def greetingcard
      Spree::Greetingcard.unscoped { super }
    end
    ...
    def set_option_value(opt_name, opt_value)
      ...
      unless current_value.nil?
        ...
        # then we have to check to make sure that the greetingcard has the option type
        unless self.greetingcard.option_types.include? option_type
          self.greetingcard.option_types << option_type
        end
      end
    ...
    def set_master_out_of_stock
      ...
      if greetingcard.master && greetingcard.master.in_stock?
        greetingcard.master.stock_items.update_all(backorderable: false)
        greetingcard.master.stock_items.each(&:reduce_count_on_hand_to_zero)
      end
    end

    # Ensures a new variant takes the product master price when price is not supplied
    # Ensures a new variant takes the greetingcard master price when price is not supplied
    def check_price
      if price.nil? && Spree::Config[:require_master_price]
        raise 'No master variant found to infer price' unless product && product.master
        raise 'Must supply price for variant or master.price for product.' if self == product.master
        self.price = product.master.price
        raise 'No master variant found to infer price' unless greetingcard && greetingcard.master
        raise 'Must supply price for variant or master.price for greetingcard.' if self == greetingcard.master
        self.price = greetingcard.master.price
      end
