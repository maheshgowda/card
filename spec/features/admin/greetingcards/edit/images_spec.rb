...
describe "Greetingcard Images", type: :feature, js: true do
  stub_authorization!
  ...
  context "uploading, editing, and deleting an image" do
    it "should allow an admin to upload and edit an image for a greetingcard" do
  ...
  it "should see variant images", js: false do
    ...
    visit spree.admin_greetingcard_images_path(variant.greetingcard)
  ...
  it "should not see variant column when greetingcard has no variants", js: false do
    greetingcard = create(:greetingcard)
    greetingcard.images.create!(attachment: File.open(file_path))
    visit spree.admin_greetingcard_images_path(greetingcard)
