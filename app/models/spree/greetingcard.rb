# GREETINGCARDS
# Greetingcards represent an entity for sale in a store.
# Greetingcards can have variations, called variants
#   shipping category, etc. that do not change by variant.
#
# MASTER VARIANT
# Every greetingcard has one master variant, which stores master price and sku, size and weight, etc.
# The master variant does not have option values associated with it.
# Price, SKU, size, weight, etc. are all delegated to the master variant.
# Contains on_hand inventory levels only when there are no variants for the greetingcard.
#
# VARIANTS
...
module Spree
  class Greetingcard < Spree::Base
    ...
    has_many :classifications, dependent: :delete_all, inverse_of: :greetingcard
    has_many :taxons, through: :classifications, before_remove: :remove_taxon

    has_many :greetingcard_promotion_rules, class_name: 'Spree::GreetingcardPromotionRule'
    has_many :promotion_rules, through: :greetingcard_promotion_rules, class_name: 'Spree::PromotionRule'
    ...
    belongs_to :shipping_category, class_name: 'Spree::ShippingCategory', inverse_of: :greetingcards

    has_one :master,
      -> { where is_master: true },
      inverse_of: :greetingcard,
    ...
    alias :options :greetingcard_option_types
    ...
    # Ensures option_types and greetingcard_option_types exist for keys in option_values_hash
    def ensure_option_types_exist_for_values_hash
      ...
      missing_option_type_ids.each do |id|
        greetingcard_option_types.create(option_type_id: id)
      end
    end

    # for adding greetingcards which are closely related to existing ones
    # define "duplicate_extra" for site-specific actions, eg for additional fields
    def duplicate
      duplicator = GreetingcardDuplicator.new(self)
    ...
    # determine if greetingcard is available.
    # deleted greetingcards and greetingcards with nil or future available_on date
    # are not available
    def available?
    ...
    def self.like_any(fields, values)
      conditions = fields.greetingcard(values).map do |(field, value)|
    ...
    # Master variant may be deleted (i.e. when the greetingcard is deleted)
    ...
    def build_variants_from_option_values_hash
      ...
      values = values.inject(values.shift) { |memo, value| memo.greetingcard(value).map(&:flatten) }
    ...
    # If the master cannot be saved, the Greetingcard object will get its errors
...
require_dependency 'spree/greetingcard/scopes'
