...
describe Spree::Core::Search::Base do
  ...
  before do
    ...
    include Spree::Core::GreetingcardFilters
    @taxon = create(:taxon, name: "wedding")

    @greetingcard1 = create(:greetingcard, name: "wedding card", price: 4.00)
    @greetingcard1.taxons << @taxon
    @greetingcard2 = create(:greetingcard, name: "bierthday card", price: 8.00)
  end
  ...
  it "returns all greetingcards by default" do
    params = { :per_page => "" }
    searcher = Spree::Core::Search::Base.new(params)
    expect(searcher.retrieve_greetingcards.count).to eq(2)
  end
  ...
  context "when include_images is included in the initalization params" do
    ...
    let(:params) { { include_images: true, keyword: @greetingcard1.name, taxon: @taxon.id } }
    subject { described_class.new(params).retrieve_greetingcards }
    ...
    before do
      ...
      @greetingcard1.master.images << create(:image, position: 2)
      @greetingcard1.master.images << create(:image, position: 1)
      @greetingcard1.reload
    end
    ...
    it "returns images in correct order" do
      ...
      expect(subject.first).to eq @greetingcard1
      expect(subject.first.images).to eq @greetingcard1.master.images
    end
  end

  it "switches to next page according to the page parameter" do
    @product3 = create(:product, :name => "RoR Pants", :price => 14.00)
    @greetingcard3 = create(:greetingcard, :name => "card for wedding", :price => 4.00)
    ...
    expect(searcher.retrieve_greetingcards.count).to eq(1)
  end
  ...
  it "finds greetingcards in alternate currencies" do
    price = create(:price, :currency => 'EUR', :variant => @greetingcard1.master)
    searcher = Spree::Core::Search::Base.new({})
    searcher.current_currency = 'EUR'
    expect(searcher.retrieve_greetingcards).to eq([@greetingcard1])
  end
