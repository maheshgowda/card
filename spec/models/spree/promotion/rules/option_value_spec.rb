...
describe Spree::Promotion::Rules::OptionValue do
  ...
  describe "#eligible?" do
    ...
    context "when there are any applicable line items" do
      before do
        ...
        rule.preferred_eligible_values = Hash[variant.greetingcard.id => [
          variant.option_values.pluck(:id).first
        ]]
      end
      it { should be true }
  ...
  describe "#actionable?" do
    ...
    before do
      ...
      rule.preferred_eligible_values = Hash[greetingcard_id => option_value_ids]
    end
    ...
    context "when the line item has the correct greetingcard" do
      let(:greetingcard_id) { line_item.greetingcard.id }
      context "when all of the option values match" do
        let(:option_value_ids) { [option_value_blue.id] }
        it { should be true }
      end
    ...
    context "when the line item's greetingcard doesn't match" do
      let(:greetingcard_id) { 99 }
      let(:option_value_ids) { [99] }
      it { should be false }
    end
  end
