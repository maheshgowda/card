...
describe Spree::Promotion, :type => :model do
  ...
  context "#greetingcards" do
    let(:greetingcard) { create(:greetingcard) }
    let(:promotion) { create(:promotion) }

    context "when it has greetingcard rules with greetingcards associated" do
      let(:promotion_rule) { create(:promotion_rule, promotion: promotion, type: 'Spree::Promotion::Rules::Greetingcard') }

      before do
        promotion.promotion_rules << promotion_rule
        greetingcard.greetingcard_promotion_rules.create(promotion_rule_id: promotion_rule.id)
      end

      it "should have greetingcards" do
        expect(promotion.reload.greetingcards.size).to eq(1)
      end
    end

    context "when there's no greetingcard rule associated" do
      it "should not have greetingcards but still return an empty array" do
        expect(promotion.greetingcards).to be_blank
      end
    end
  end
  ...
  context "#eligible?" do
    ...
    context "when promotable is a Spree::LineItem" do
      ...
      let(:greetingcard) { promotable.greetingcard }
      before do
        product.promotionable = promotionable
        greetingcard.promotionable = promotionable
      end
      ...
      context "and greetingcard is promotionable" do
        let(:promotionable) { true }
        it { is_expected.to be true }
      end
      ...
      context "and greetingcard is not promotionable" do
        let(:promotionable) { false }
        it { is_expected.to be false }
      end
    end
      ...
      context "and it contains items" do
        ...
        context "and the items are all non-promotionable" do
          before do
            ...
            line_item.greetingcard.update_column(:promotionable, false)
          end
          it { is_expected.to be false }
        end
