  ...
  context '#elegible?(order)' do
    ...
    context 'with any match policy' do
      ...
      it 'is eligible if order does has any prefered taxon' do
        ...
        order.greetingcards.first.taxons << taxon
        rule.taxons << taxon
        expect(rule).to be_eligible(order)
      end
      ...
      context 'when order contains items from different taxons' do
        ...
        it 'should act on a greetingcard within the eligible taxon' do
          expect(rule).to be_actionable(order.line_items.last)
        end
        ...
        it 'should not act on a greetingcard in another taxon' do
          order.line_items << create(:line_item, greetingcard: create(:greetingcard, taxons: [taxon2]))
          expect(rule).not_to be_actionable(order.line_items.last)
        end
      end
      ...
      context "when order does not have any prefered taxon" do
        ...
        it "sets an error message" do
          ...
          expect(rule.eligibility_errors.full_messages.first).
            ...
            to eq "You need to add a greetingcard from an applicable category before applying this coupon code."
        end
      end
      ...
      context 'when a greetingcard has a taxon child of a taxon rule' do
        before do
          taxon.children << taxon2
          order.greetingcards.first.taxons << taxon2
          rule.taxons << taxon2
        end
    ...
    context 'with all match policy' do
      ...
      it 'is eligible order has all prefered taxons' do
        ...
        order.greetingcards.first.taxons << taxon2
        order.greetingcards.last.taxons << taxon
      ...
      context "when order does not have all prefered taxons" do
        ...
        it "sets an error message" do
          ...
          expect(rule.eligibility_errors.full_messages.first).
            ...
            to eq "You need to add a greetingcard from all applicable categories before applying this coupon code."
        end
      end
      ...
      context 'when a greetingcard has a taxon child of a taxon rule' do
        ...
        before do
          taxon.children << taxon2
          order.greetingcards.first.taxons << taxon2
          order.greetingcards.last.taxons << taxon3
          rule.taxons << taxon2
          rule.taxons << taxon3
        end
