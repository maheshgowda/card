...
module Spree
  describe GreetingcardsHelper, :type => :helper do
    include GreetingcardsHelper
    ...
    context "#variant_price_diff" do
      let(:greetingcard_price) { 10 }
      let(:variant_price) { 10 }

      before do
        @variant = create(:variant, :greetingcard => greetingcard)
        greetingcard.price = 15
        @variant.price = 10
        allow(greetingcard).to receive(:amount_in) { greetingcard_price }
    ...
    context "#variant_price_full" do
      before do
        ...
        @variant1 = create(:variant, :greetingcard => greetingcard)
        @variant2 = create(:variant, :greetingcard => greetingcard)
      end
      ...
      context "when currency is JPY" do
        ...
        before do
          greetingcard.variants.active.each do |variant|
    ...
    context "#greetingcard_description" do
      # Regression test for #1607
      it "renders a greetingcard description without excessive paragraph breaks" do
        greetingcard.description = %Q{
...
<li>Lorem ipsum dolor sit amet</li>
        ...
        description = greetingcard_description(greetingcard)
        expect(description.strip).to eq(greetingcard.description.strip)
      end

      it "renders a greetingcard description with automatic paragraph breaks" do
        greetingcard.description = %Q{
THIS IS THE BEST GREETINGCARD EVER!

"IT CHANGED MY LIFE" - Sue, MD}

        description = greetingcard_description(greetingcard)
        expect(description.strip).to eq(%Q{<p>\nTHIS IS THE BEST GREETINGCARD EVER!</p>"IT CHANGED MY LIFE" - Sue, MD})
      end

      it "renders a greetingcard description without any formatting based on configuration" do
        ...
        greetingcard.description = initialDescription

        Spree::Config[:show_raw_greetingcard_description] = true
        description = greetingcard_description(greetingcard)
        expect(description).to eq(initialDescription)
      end

      context "renders a greetingcard description default description incase description is blank" do
        before { greetingcard.description = '' }
        it { expect(greetingcard_description(greetingcard)).to eq(Spree.t(:greetingcard_has_no_description)) }
      end
    end
    ...
    shared_examples_for "line item descriptions" do
      context 'variant has a blank description' do
        let(:description) { nil }
        it { is_expected.to eq(Spree.t(:greetingcard_has_no_description)) }
      end
    ...
    context '#cache_key_for_greetingcards' do
      let(:zone) { Spree::Zone.new }
      let(:price_options) { { tax_zone: zone } }

      subject { helper.cache_key_for_greetingcards }
      before(:each) do
        @greetingcards = double('greetingcards collection')
      ...
      context 'when there is a maximum updated date' do
        ...
        before :each do
          allow(@greetingcards).to receive(:count) { 5 }
          allow(@greetingcards).to receive(:maximum).with(:updated_at) { updated_at }
        end

        it { is_expected.to eq("en/USD/spree/zones/new/spree/greetingcards/all-10-20111213-5") }
      end

      context 'when there is no considered maximum updated date' do
        let(:today) { Date.new(2013, 12, 11) }
        before :each do
          allow(@greetingcards).to receive(:count) { 1234567 }
          allow(@greetingcards).to receive(:maximum).with(:updated_at) { nil }
          allow(Date).to receive(:today) { today }
        end

        it { is_expected.to eq("en/USD/spree/zones/new/spree/greetingcards/all-10-20131211-1234567") }
      end
    end

    context "#cache_key_for_greetingcard" do
      let(:greetingcard) { Spree::Greetingcard.new }
      let(:price_options) { { tax_zone: zone } }

      subject(:cache_key) { helper.cache_key_for_greetingcard(greetingcard) }
      ...
      context "when there is a current tax zone" do
        ...
        it "includes the current_tax_zone" do
          is_expected.to eq("en/USD/spree/zones/new/spree/greetingcards/new/")
        end
      end
      ...
      context "when there is no current tax zone" do
        ...
        it { is_expected.to eq("en/USD/spree/greetingcards/new/") }
      end
      ...
      context "when current_price_options includes nil values" do
        ...
        it "does not include nil values" do
          expect(cache_key).to eq("en/USD/spree/zones/new/spree/greetingcards/new/")
        end
      end
      ...
      context "when current_price_options includes values that do not implement cache_key" do
        ...
        it "includes string representations of these values" do
          expect(cache_key).to eq("en/USD/true/spree/zones/new/spree/greetingcards/new/")
        end
      end
