    ...
    describe Coupon, :type => :model do
      ...
      context "existing coupon code promotion" do
        ...
        context "for an order with taxable line items" do
          ...
          context "and the greetingcard price is less than promo discount" do
            before(:each) do
              3.times do |i|
                taxable = create(:greetingcard, :tax_category => @category, :price => 9.0)
                @order.contents.add(taxable.master, 1)
              end
            end
            it "successfully applies the promo" do
          ...
          context "and the greetingcard price is greater than promo discount" do
            before(:each) do
              3.times do |i|
                taxable = create(:greetingcard, :tax_category => @category, :price => 11.0)
                @order.contents.add(taxable.master, 2)
              end
            end
            it "successfully applies the promo" do
          ...
          context "and multiple quantity per line item" do
            before(:each) do
              ...
              3.times do |i|
                ...
                taxable = create(:greetingcard, :tax_category => @category, :price => 10.0)
                @order.contents.add(taxable.master, 2)
              end
            end
            it "successfully applies the promo" do
