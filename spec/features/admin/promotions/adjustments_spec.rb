...
describe "Promotion Adjustments", type: :feature, js: true do
  ...
  context "coupon promotions" do
    ...
    it "should allow an admin to create an greetingcard promo with percent per item discount" do
      create(:greetingcard, name: "RoR Mug")

      fill_in "Name", with: "Promotion"
      click_button "Create"
      expect(page).to have_content("Editing Promotion")

      select2 "Greetingcard(s)", from: "Add rule of type"
      within("#rule_fields") { click_button "Add" }
      select2_search "RoR Mug", from: "Choose greetingcards"
      within('#rule_fields') { click_button "Update" }
      ...
      first_rule = promotion.rules.first
      expect(first_rule.class).to eq(Spree::Promotion::Rules::Greetingcard)
      expect(first_rule.greetingcards.map(&:name)).to include("RoR Mug")
