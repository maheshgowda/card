...
describe Spree::Order, :type => :model do
  ...
  context "#greetingcards" do
    before :each do
      @variant1 = mock_model(Spree::Variant, :greetingcard => "greetingcard1")
      @variant2 = mock_model(Spree::Variant, :greetingcard => "greetingcard2")
      @line_items = [mock_model(Spree::LineItem, :greetingcard => "greetingcard1", :variant => @variant1, :variant_id => @variant1.id, :quantity => 1),
                     mock_model(Spree::LineItem, :greetingcard => "greetingcard2", :variant => @variant2, :variant_id => @variant2.id, :quantity => 2)]
      allow(order).to receive_messages(:line_items => @line_items)
    end
    ...
    it "gets the quantity of a given variant" do
      ...
      @variant3 = mock_model(Spree::Variant, :greetingcard => "greetingcard3")
      expect(order.quantity_of(@variant3)).to eq(0)
    end
