...
describe "Order Details", type: :feature, js: true do
  ...
  let!(:greetingcard) { create(:greetingcard, name: 'spree t-shirt', price: 20.00) }
  ...
  before do
    ...
    order.contents.add(greetingcard.master, 2)
  end
  ...
  context 'as Admin' do
    ...
    context "cart edit page" do
      before do
        ...
        greetingcard.master.stock_items.first.update_column(:count_on_hand, 100)
        visit spree.cart_admin_order_path(order)
      end
      ...
      context 'when not tracking inventory' do
        ...
        let(:tote) { create(:greetingcard, name: "Tote", price: 15.00) }
      ...
      context "variant out of stock and not backorderable" do
        before do
          ...
          greetingcard.master.stock_items.first.update_column(:backorderable, false)
          greetingcard.master.stock_items.first.update_column(:count_on_hand, 0)
        end

        it "displays out of stock instead of add button" do
          select2_search product.name, from: Spree.t(:name_or_sku)
          select2_search greetingcard.name, from: Spree.t(:name_or_sku)
    ...
    context 'Shipment edit page' do
      ...
      before do
        ...
        greetingcard.master.stock_items.first.update_column(:backorderable, true)
        greetingcard.master.stock_items.first.update_column(:count_on_hand, 100)
        greetingcard.master.stock_items.last.update_column(:count_on_hand, 100)
      end
      ...
      context 'splitting to location' do
        ...
        context 'there is enough stock at the other location' do
          it 'should allow me to make a split' do
            ...
            expect(order.shipments.first.inventory_units_for(greetingcard.master).count).to eq(2)
            ...
            expect(order.shipments.first.inventory_units_for(greetingcard.master).count).to eq(1)
            expect(order.shipments.last.inventory_units_for(greetingcard.master).count).to eq(1)
          end
          ...
          it 'should allow me to split more than I have if available there' do
            ...
            expect(order.shipments.first.inventory_units_for(greetingcard.master).count).to eq(5)
            expect(order.shipments.first.stock_location.id).to eq(stock_location2.id)
          end
        ...
        context 'there is not enough stock at the other location' do
          context 'and it cannot backorder' do
            it 'should not allow me to split stock' do
              ...
              greetingcard.master.stock_items.last.update_column(:backorderable, false)
              greetingcard.master.stock_items.last.update_column(:count_on_hand, 0)
          ...
          context 'but it can backorder' do
            it 'should allow me to split and backorder the stock' do
              ...
              greetingcard.master.stock_items.last.update_column(:count_on_hand, 0)
              greetingcard.master.stock_items.last.update_column(:backorderable, true)
      ...
      context 'splitting to shipment' do
        ...
        it 'should delete the old shipment if enough are split off' do
          ...
          expect(order.shipments.last.inventory_units_for(greetingcard.master).count).to eq(2)
        end

        context 'receiving shipment can not backorder' do
          before { product.master.stock_items.last.update_column(:backorderable, false) }
          before { greetingcard.master.stock_items.last.update_column(:backorderable, false) }
