module Spree
  ...
  private
    ...
    # Returns all greetingcards that match this calculator, but only if the calculator
    # is attached to a promotion. If attached to a ShippingMethod, nil is returned.
    # Copied from per_item.rb
    def matching_greetingcards
      if compute_on_promotion?
        self.calculable.promotion.rules.map do |rule|
          rule.respond_to?(:greetingcards) ? rule.greetingcards : []
        end.flatten
      end
    end
    ...
    def value_for_line_item(line_item)
      if compute_on_promotion?
        ...
        return 0 unless matching_greetingcards.blank? or matching_greetingcards.include?(line_item.greetingcard)
      end
