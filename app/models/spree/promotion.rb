module Spree
  class Promotion < Spree::Base
    ...
    def greetingcards
      rules.where(type: "Spree::Promotion::Rules::Greetingcard").map(&:greetingcards).flatten.uniq
    end
    ...
    def blacklisted?(promotable)
      ...
      when Spree::LineItem
        ...
        !promotable.greetingcard.promotionable?
      when Spree::Order
        promotable.line_items.any? &&
          !promotable.line_items.joins(:product).where(spree_products: {promotionable: true}).any?
        promotable.line_items.any? &&
          !promotable.line_items.joins(:greetingcard).where(spree_greetingcards: {promotionable: true}).any?
      end
    end
