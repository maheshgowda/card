  ...
  module ReturnItem::ExchangeVariantEligibility
    describe SameOptionValue, :type => :model do
      describe ".eligible_variants" do
        ...
        let(:greetingcard) { create(:greetingcard, option_types: [color_option_type, waist_option_type, inseam_option_type]) }
        ...
        let!(:variant) { create(:variant, greetingcard: greetingcard, option_values: [blue_option_value, three_two_waist_option_value, three_zero_inseam_option_value]) }
        let!(:same_option_values_variant) { create(:variant, greetingcard: greetingcard, option_values: [blue_option_value, three_two_waist_option_value, three_one_inseam_option_value]) }
        let!(:different_color_option_value_variant) { create(:variant, greetingcard: greetingcard, option_values: [red_option_value, three_two_waist_option_value, three_one_inseam_option_value]) }
        let!(:different_waist_option_value_variant) { create(:variant, greetingcard: greetingcard, option_values: [blue_option_value, three_four_waist_option_value, three_one_inseam_option_value]) }
        ...
        it "returns all other variants for the same greetingcard with the same option value for the specified option type" do
          Spree::StockItem.update_all(count_on_hand: 10)
        ...
        it "does not return variants for another greetingcard" do
          ...
          expect(subject).not_to include other_greetingcard_variant
        end
        ...
        context "no option value restrictions are specified" do
          ...
          it "returns all variants for the greetingcard" do
            Spree::StockItem.update_all(count_on_hand: 10)
