    ...
    module Rules
      class Taxon < PromotionRule
        ...
        def actionable?(line_item)
          ...
          taxon_greetingcard_ids.include? line_item.variant.greetingcard_id
        end
        ...
        def order_taxons(order)
          ...
          Spree::Taxon.joins(greetingcards: {variants_including_master: :line_items}).where(spree_line_items: {order_id: order.id}).uniq
        end
        ...
        def taxon_greetingcard_ids
          Spree::Greetingcard.joins(:taxons).where(spree_taxons: {id: taxons.pluck(:id)}).pluck(:id).uniq
        end
      end
    end
  end
