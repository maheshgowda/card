...
describe Spree::Variant, :type => :model do
  ...
  context "after create" do
    ...
    context "stock location has disable propagate all variants" do
      ...
      it "propagate to stock items" do
        ...
        greetingcard.variants.create(:name => "Foobar")
      end
    end
    ...
    describe 'mark_master_out_of_stock' do
      before do
        ...
        greetingcard.master.stock_items.first.set_count_on_hand(5)
      end
      context 'when product is created without variants but with stock' do
        it { expect(product.master).to be_in_stock }
      end
      
      context 'when greetingcard is created without variants but with stock' do
        it { expect(greetingcard.master).to be_in_stock }
      end

      context 'when a variant is created' do
        let!(:new_variant) { create(:variant, product: product) }
        let!(:new_variant) { create(:variant, greetingcard: greetingcard) }

        it { expect(product.master).to_not be_in_stock }
        it { expect(greetingcard.master).to_not be_in_stock }
      end
    end
  end
  ...
  context "greetingcard has other variants" do
    describe "option value accessors" do
      before {
        @multi_variant = FactoryGirl.create :variant, :greetingcard => variant.greetingcard
        variant.greetingcard.reload
      }
    ...
    context "greetingcard has other variants" do
      describe "option value accessors" do
        before {
          @multi_variant = create(:variant, :greetingcard => variant.greetingcard)
          variant.greetingcard.reload
        }
  ...
  describe '#tax_category' do
    context 'when tax_category is nil' do
      ...
      let(:greetingcard) { build(:greetingcard) }
      let(:variant) { build(:variant, product: product, tax_category_id: nil) }
      let(:variant) { build(:variant, greetingcard: greetingcard, tax_category_id: nil) }
      it 'returns the parent products tax_category' do
        expect(variant.tax_category).to eq(product.tax_category)
      end
      it 'returns the parent greetingcards tax_category' do
        expect(variant.tax_category).to eq(greetingcard.tax_category)
      end
    end
  ...
  describe "touching" do
    ...
    it "updates a greetingcard" do
      variant.greetingcard.update_column(:updated_at, 1.day.ago)
      variant.touch
      expect(variant.greetingcard.reload.updated_at).to be_within(3.seconds).of(Time.current)
    end
  ...
  describe "#available?" do
    ...
    context 'when discontinued' do
      ...
      context 'when greetingcard is available' do
        before(:each) do
          allow(variant.greetingcard).to receive(:available?) { true }
        end
      ...
      context 'when greetingcard is not available' do
        before(:each) do
          allow(variant.greetingcard).to receive(:available?) { false }
        end
  ...
  describe "#check_price" do
    ...
    context 'require_master_price set true' do
      ...
      context 'greetingcard and master_variant present and equal' do
        context 'price nil and currency present' do
          before { variant.price = nil }
          it { expect(variant.send(:check_price)).to be(nil) }
          ...
          context 'check variant price' do
            before { variant.send(:check_price) }
            it { expect(variant.price).to eq(variant.greetingcard.master.price) }
          end
        end
      ...
      context 'greetingcard not present' do
        context 'greetingcard not present' do
          before { variant.greetingcard = nil }
